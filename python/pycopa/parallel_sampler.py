...
run_sampler_c = getattr(
    pycopa_c,
    "pycopa__parallel_sampler_run_sampler_c")
...
def run_sampler(
        ...
        nwalkers=50, nsteps=1000, nthreads=2):
    ...
    nthreads_c = ctypes.c_int(nthreads)
    walkers = np.empty(
        (ndim, nwalkers, nthreads),
        dtype=np.float64,
        order='F')
    walkers_c = walkers.ctypes.data_as(
        ctypes.POINTER(ctypes.c_double))
    chains = np.empty(
        (ndim, nwalkers, nsteps, nthreads),
        dtype=np.float64,
        order='F')
    ...
    run_sampler_c(
        ...
        nwalkers_c, nsteps_c, nthreads_c,
