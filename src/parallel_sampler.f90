module pycopa__parallel_sampler
...
contains
  ...
  subroutine run_sampler_c(  &
    ...
    bind(C, name="pycopa__parallel_sampler_run_sampler_c")
    ...
    real(c_double), intent(out) :: walkers(ndim, nwalkers, nthreads)
    real(c_double), intent(out) :: chains(ndim, nwalkers, nsteps, nthreads)
    real(c_double), intent(out) :: log_probs(nwalkers, nsteps, nthreads)
    ...
    do k = 1, nthreads
      do j = 1, nwalkers
        do i = 1, ndim
          walkers(i, j, k) = walkers_wp(i, j, k)
        end do
      end do
    end do
    ...
    do l = 1, nthreads
      do k = 1, nsteps
        do j = 1, nwalkers
          do i = 1, ndim
            chains(i, j, k, l) = chains_wp(i, j, k, l)
          end do
        end do
      end do
    end do
    ...
    do k = 1, nthreads
      do j = 1, nsteps
        do i = 1, nwalkers
          log_probs(i, j, k) = log_probs_wp(i, j, k)
        end do
      end do
    end do
...
end module pycopa__parallel_sampler
